...
description: Step-by-step instructions for setting up ACK NestJS Boilerplate on your development environment
---

import { Download, Docker, Database, Settings, CheckCircle, BookOpen, Cog } from 'lucide-react';
...
<Callout title="Quick Setup Guide" type="info">
  ACK NestJS Boilerplate can be installed and configured in minutes using Docker or manual setup with Node.js, MongoDB, and Redis dependencies.
</Callout>
...
<Cards>
    <Card type="link" href="/docs/environment" icon={<Settings />}>
        Environment Documentation - Complete environment variable configuration
    </Card>

    <Card type="link" href="/docs/database" icon={<Database />}>
        Database Documentation - Database setup and migration details
    </Card>

    <Card type="link" href="/docs/configuration" icon={<Cog />}>
        Configuration Documentation - Understanding configuration structure
    </Card>
</Cards>
...
<InlineTOC
    ...
    items={[
        ...
        },
        {
            ...
            url: '#-standard-installation',
            depth: 2,
        },
        {
            title: 'Install Packages',
            url: '#install-packages',
            depth: 3,
        },
        {
        ...
        },
        {
            ...
            url: '#docker-specific-configuration',
            depth: 3,
        },
        {
            title: 'Generate Keys',
        ...
        {
            title: 'Generate Database Client',
            url: '#generate-database-client',
            depth: 2,
        },
        {
            title: 'Database Migration & Seeding',
            url: '#database-migration--seeding',
            depth: 2,
        },
        {
            title: 'Run Project',
...
```bash title="Generate JWT key pairs"
...
```bash title="Update JWKS URLs in .env"
...
```bash title="Copy environment file for Docker"
# Copy the example environment file
cp .env.example .env
```
...
```bash title="Generate keys for Docker"
...
```bash title="Monitor Docker containers"
...
```bash title="Migrate database schema"
pnpm db:migrate
```

**Seed all initial data:**
```bash title="Seed all initial data"
pnpm migration:seed
```
...
```bash title="Seed email templates"
...
```bash title="Build and run production"
...
```bash title="Code formatting and linting"
...
```bash title="Dependency management"
