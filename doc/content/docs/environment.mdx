...
description: Complete guide to environment variable configuration and management
---

import { Lock, Settings, AlertCircle, CheckCircle, Database } from 'lucide-react';
...
<Cards>
    <Card type="link" href="environment" icon={<Settings />}>
        Environment Documentation - Environment variable configuration
    </Card>

    <Card type="link" href="database" icon={<Database />}>
        Database Documentation - Database setup and initialization
    </Card>
</Cards>
...
<Callout title="Environment Configuration" type="info">
  ACK NestJS Boilerplate uses `.env` files with comprehensive environment variable validation through `@nestjs/config` and class-validator for type-safe configuration management across development, staging, and production environments.
</Callout>
